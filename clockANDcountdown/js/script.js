...
let second = 1000; // in miliseconds
let minute = second * 60;
let hour = minute * 60;
let day = hour * 24;
let finalDate = new Date("Jun 6, 2023 00:00:00");

let startClock = () => {
  updateTime();
  updateCountdown(); // set the nitial value for the countdown timer
  setInterval(() => {
    updateTime();
    updateCountdown();
  }, 1000);
...
let updateTime = () => {
  // arrow function
  let now = new Date();
  let hours = now.getHours() % 12; // converting 24 hours into 12 hours
...
let updateCountdown = () => {
  let now = new Date(); // find current date
  let diff = finalDate - now;
  let diffObj = convertMillisToDHMS(diff);

  // below is the use of Ternary operator
  // WHY - if the data is smaller than 9 ( 03 sec, 07 min)
...
// create a function which will convern miliseconds into
...
let convertMillisToDHMS = (millis) => {
  ...
  return { days, hours, minutes, seconds }; // ES2020 js syntax
};
startClock();
